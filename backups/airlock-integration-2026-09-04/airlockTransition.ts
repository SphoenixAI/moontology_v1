...
export class AirlockTransitionController {
  ...
  public enterScene2(): void {
    if (this.transitionStarted || this.state === 'scene2') {
      return;
    }
    if (!this.approachReached) {
      this.reachApproach();
  ...
  private async performWorld2Transition(): Promise<void> {
    ...
    const scene2World = await this.options.loadWorldLabsWorld(
      this.options.scene2Url,
    );

    if (scene2World.activeMode !== 'marble') {
      console.warn(
        '[WORLD2] World Labs visual unavailable; continuing with loader fallback.',
        scene2World.fallbackReason,
      );
    }
