...
const { assets } = JSON.parse(await readFile(mobile ? 'docs/mobile-assets.json' : optimized ? 'docs/optimized-assets.json' : 'dist-public/map-assets-manifest.json', 'utf8'));
const receiptPath = mobile ? 'docs/mobile-public-assets.json' : optimized ? 'docs/optimized-public-assets.json' : 'docs/public-assets.json';
...
for (const asset of assets) {
  ...
  const pathname = `${mobile ? 'v3' : optimized ? 'v2' : 'v1'}/${asset.sha256.slice(0, 16)}/${asset.path}`;
  let uploaded;
